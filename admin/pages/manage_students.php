...
// Store students in array for PDF export
$students = [];
...
<script>
    ...
    function exportPDF() {
        ...
        doc.text('Generated by: Admin', 20, 35);

        // Dynamically calculate startY based on filters
        let currentY = 35;
        <?php if ($filter_course): ?>
        currentY += 5;
        doc.text('Course Filter: <?php echo addslashes($filter_course); ?>', 20, currentY);
        <?php endif; ?>
        <?php if ($search_nic): ?>
        currentY += 5;
        doc.text('NIC Search: <?php echo addslashes($search_nic); ?>', 20, currentY);
        <?php endif; ?>
        
        // Use json_encode to safely transfer PHP array to a JS variable.
        const studentsData = <?php echo json_encode($students, JSON_HEX_QUOT | JSON_HEX_APOS); ?>; 

        const tableData = [];
        
        // Helper function to clean strings for PDF cells (removes newlines and excess whitespace for better UI)
        const cleanString = (str) => {
            if (str === null || str === undefined || str === '') return 'N/A';
            return String(str).replace(/(\r\n|\n|\r)/gm, ' ').replace(/\s\s+/g, ' ').trim();
        };

        // Iterate over the safe JS array to build table data
        studentsData.forEach(student => {
            tableData.push([
                cleanString(student.full_name),
                cleanString(student.Student_id),
                cleanString(student.nic),
                cleanString(student.contact_no),
                cleanString(student.whatsapp_no),
                cleanString(student.address),
                cleanString(student.course_option_one),
                cleanString(student.course_option_two)
                // Status column is intentionally omitted here
            ]);
        });
        
        doc.autoTable({
            // Status column header is intentionally omitted here
            head: [['Name', 'Student ID', 'NIC', 'Contact', 'WhatsApp', 'Address', 'Course 1', 'Course 2']],
            body: tableData,
            // Use currentY for startY, plus a buffer
            startY: currentY + 5,
            
            // Re-optimized column widths (Total 270mm, fits page)
            columnStyles: {
                0: { cellWidth: 40 },  // Name (40)
                1: { cellWidth: 30 },  // Student ID (30)
                2: { cellWidth: 30 },  // NIC (30)
                3: { cellWidth: 25 },  // Contact (25)
                4: { cellWidth: 25 },  // WhatsApp (25)
                5: { cellWidth: 60 },  // Address (60)
                6: { cellWidth: 30 },  // Course 1 (30)
                7: { cellWidth: 30 }   // Course 2 (30)
                // Total: 270mm
            },
        ...
        const filename = 'NVTI_Students_Admin_' + new Date().toISOString().split('T')[0] + '.pdf';
        doc.save(filename);
    }

    // Attach event listener to the ID of the button after the DOM content is fully loaded
