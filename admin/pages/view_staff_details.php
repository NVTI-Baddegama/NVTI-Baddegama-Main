...
if ($result->num_rows == 1) {
    ...
    // --- NEW: Check if the staff is SPECIFICALLY an Instructor ---
...
if (isset($_SESSION['staff_success_msg'])) {
    echo '<div class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 rounded-lg mb-6" role="alert"><p class="font-bold">Success!</p><p>' . htmlspecialchars($_SESSION['staff_success_msg']) . '</p></div>';
    unset($_SESSION['staff_success_msg']);
}
if (isset($_SESSION['staff_error_msg'])) {
    echo '<div class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded-lg mb-6" role="alert"><p class="font-bold">Error!</p><p>' . htmlspecialchars($_SESSION['staff_error_msg']) . '</p></div>';
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-1">
        <div class="bg-white p-6 rounded-xl shadow-lg sticky top-28">
            ...
            <form action="../lib/staff_action_handler.php" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" id="position" name="position" required
                     ...
                     <p class="text-xs text-gray-500 mt-1">Enter the exact position name.</p>
                </div>
                ...
                <div>
                    ...
                    <select id="course_no" name="course_no"
                            ...
                            <?php if (!$is_instructor) echo 'bg-gray-100 cursor-not-allowed'; // Style if disabled ?>"
                            <?php if (!$is_instructor) echo 'disabled'; // Disable if NOT Instructor ?> >
                        <option value="">-- No Course Assigned --</option>
                        <?php
                        foreach ($courses as $course) {
                            $selected = ($is_instructor && $staff['course_no'] == $course['course_no']) ? 'selected' : ''; // Select only if instructor
                    ...
                    <?php if (!$is_instructor): // Show message if disabled ?>
