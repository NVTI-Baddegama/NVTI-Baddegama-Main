...
if (isset($_POST['submit'])) {
    ...
    */
    ...
    if ($stmt_insert->execute()) {
        ...
        $stmt_insert->close();
        $con->close();
        // Check if error is about duplicate NIC (still possible if UNIQUE KEY wasn't dropped)
        if (strpos($error_message, 'Duplicate entry') !== false && strpos($error_message, 'nic_unique') !== false) {
             header("location:../pages/student_register.php?error=NIC_Already_Registered");
        } else {
             header("location:../pages/student_register.php?error=Application_Submission_Failed");
        }
        exit();
    }
